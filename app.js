...
function renderWatched() {
  ...
  grid.innerHTML = items.map((a, idx) => {
    ...
    // Banner: custom image set by user, side by side with poster
    const bannerHtml = a.banner
      ? `<div class="card-banner">
           <img src="${a.banner}" loading="lazy" onerror="this.parentElement.style.display='none'"/>
           <button class="banner-change-btn" onclick="triggerBannerPick(${a.id},event)" title="Change banner">🖼️</button>
         </div>`
      : `<div class="card-banner card-banner-empty" onclick="triggerBannerPick(${a.id},event)" title="Add banner image">
           <span>＋</span><small>Banner</small>
         </div>`;
    return `<div class="anime-card" style="animation-delay:${idx * 0.04}s">
      <input type="file" accept="image/*" id="bannerFile_${a.id}" style="display:none" onchange="setBanner(${a.id},event)"/>
      <div class="card-inner">
        ...
        <div class="card-info">
          ...
          <div class="card-actions-new">
            <button class="card-action-btn" onclick="openEditModal(${a.id})">✏️</button>
            ${a.malId ? `<button class="card-action-btn success" onclick="window.open('https://myanimelist.net/anime/${a.malId}','_blank')">🔗</button>` : ''}
...
/* ── BANNER UPLOAD ── */
function triggerBannerPick(id, e) {
  e && e.stopPropagation();
  const inp = document.getElementById('bannerFile_' + id);
  if (inp) inp.click();
}
function setBanner(id, e) {
  const file = e.target.files[0]; if (!file) return;
  const reader = new FileReader();
  reader.onload = ev => {
    const a = watchedList.find(x => x.id === id);
    if (!a) return;
    a.banner = ev.target.result;
    saveWatched();
    renderWatched();
...
/* ── WALLPAPER (AnimeVault) ── */
function setAvWp(e) {
  const file = e.target.files[0]; if (!file) return;
  const reader = new FileReader();
  reader.onload = ev => {
    const data = ev.target.result;
    localStorage.setItem('av_wp', data);
    applyAvWp(data);
    const prev = document.getElementById('avWpPrev');
    if (prev) prev.innerHTML = `<img src="${data}"/>`;
    toast('🖼️ Wallpaper set!', 'success');
  };
  reader.readAsDataURL(file);
}
function applyAvWp(data) {
  document.body.style.backgroundImage = `url('${data}')`;
  document.body.classList.add('has-wp');
}
function removeAvWp() {
  localStorage.removeItem('av_wp');
  document.body.style.backgroundImage = '';
  document.body.classList.remove('has-wp');
  const prev = document.getElementById('avWpPrev');
  if (prev) prev.innerHTML = '<span id="avWpTxt">📁 Tap to choose image from gallery</span>';
  toast('🗑️ Wallpaper removed', 'info');
}
function setAvWpOp(val) {
  const el = document.getElementById('avWpOpVal');
  if (el) el.textContent = val + '%';
  document.getElementById('wpOverlay').style.background = `rgba(11,11,18,${val/100})`;
  localStorage.setItem('av_wpo', val);
}
function loadAvWp() {
  const wp  = localStorage.getItem('av_wp');
  const wpo = localStorage.getItem('av_wpo') || '80';
  if (wp) {
    applyAvWp(wp);
    const prev = document.getElementById('avWpPrev');
    if (prev) prev.innerHTML = `<img src="${wp}"/>`;
  }
  const opEl = document.getElementById('avWpOp');
  if (opEl) opEl.value = wpo;
  setAvWpOp(wpo);
}

/* ── CUSTOMIZE (open — also load wp preview) ── */
let czColor1 = '#e63462', czColor2 = '#ff9f43', czStyle = 'rounded';

function openCustomize() {
  // Load saved values
  const saved = JSON.parse(localStorage.getItem('av_customize') || '{}');
  document.getElementById('czSiteName').value = saved.siteName || '';
  document.getElementById('czTagline').value  = saved.tagline  || '';
  czColor1 = saved.color1 || '#e63462';
  czColor2 = saved.color2 || '#ff9f43';
  czStyle  = saved.style  || 'rounded';
  // Reflect in UI
  document.querySelectorAll('#colorGrid .color-swatch').forEach(s => {
    s.classList.toggle('on', s.dataset.c1 === czColor1);
  });
  document.querySelectorAll('.style-opt').forEach(s => {
    s.classList.toggle('on', s.dataset.style === czStyle);
  });
...
function czBgClose(e) { if (e.target === document.getElementById('customizeModal')) closeCustomize(); }

function pickColor(el) {
  document.querySelectorAll('#colorGrid .color-swatch').forEach(s => s.classList.remove('on'));
  el.classList.add('on');
  czColor1 = el.dataset.c1; czColor2 = el.dataset.c2;
}
function pickStyle(el) {
  document.querySelectorAll('.style-opt').forEach(s => s.classList.remove('on'));
  el.classList.add('on');
  czStyle = el.dataset.style;
}

function applyCustomize() {
  const name    = document.getElementById('czSiteName').value.trim();
  const tagline = document.getElementById('czTagline').value.trim();
  const saved   = { siteName: name, tagline, color1: czColor1, color2: czColor2, style: czStyle };
  localStorage.setItem('av_customize', JSON.stringify(saved));
  applySavedCustomize(saved);
  closeCustomize();
  toast('✨ Customization applied!', 'success');
}

function applySavedCustomize(cfg) {
  if (!cfg) return;
  // Colors
  if (cfg.color1) {
    document.documentElement.style.setProperty('--accent',  cfg.color1);
    document.documentElement.style.setProperty('--accent2', cfg.color2 || cfg.color1);
    document.documentElement.style.setProperty('--glow-r',  cfg.color1 + '44');
  }
  // Site name
  if (cfg.siteName) {
    const el = document.querySelector('.nav-logo-text');
    if (el) el.textContent = cfg.siteName;
  }
  // Card style
  const body = document.body;
  body.classList.remove('style-rounded','style-sharp','style-glass');
  if (cfg.style) body.classList.add('style-' + cfg.style);
}
